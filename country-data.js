export const PRINTIFY_COUNTRIES = [
  ...
  ["KE", "Kenya"], ["KI", "Kiribati"], ["KR", "Korea, Republic of"], ["KW", "Kuwait"],
  ...
  ["QA", "Qatar"], ["RE", "Réunion"], ["RO", "Romania"], ["RW", "Rwanda"],
  ...
  ["UG", "Uganda"], ["AE", "United Arab Emirates"], ["GB", "United Kingdom"],
...
const COUNTRY_CODE_BY_NAME = new Map(
  PRINTIFY_COUNTRIES.map(([code, name]) => [name.toLowerCase(), code])
);

const LEGACY_COUNTRY_ALIASES = new Map([
  ["usa", "US"], ["u.s.", "US"], ["united states of america", "US"],
  ["uk", "GB"], ["great britain", "GB"], ["south korea", "KR"],
  ["czech republic", "CZ"], ["turkey", "TR"]
]);

export function normalizePrintifyCountryCode(value) {
  const normalized = String(value || "").trim();
  if (!normalized) return "";
  const uppercase = normalized.toUpperCase();
  if (PRINTIFY_COUNTRIES.some(([code]) => code === uppercase)) return uppercase;
  return COUNTRY_CODE_BY_NAME.get(normalized.toLowerCase())
    || LEGACY_COUNTRY_ALIASES.get(normalized.toLowerCase())
    || "";
}

export function isPrintifyCountryCode(value) {
  return Boolean(normalizePrintifyCountryCode(value));
}
